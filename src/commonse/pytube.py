...
class CylindricalShellProperties(object):
    ...
    def __init__(self, nFull):
        ...
        self.Az = np.zeros(nFull) # self.add_output('Az', np.zeros(nFull), units='m**2', desc='cross-sectional area')
        self.Asx = np.zeros(nFull) # self.add_output('Asx', np.zeros(nFull), units='m**2', desc='x shear area')
        self.Asy = np.zeros(nFull) # self.add_output('Asy', np.zeros(nFull), units='m**2', desc='y shear area')
        self.Jz = np.zeros(nFull) # self.add_output('Jz', np.zeros(nFull), units='m**4', desc='polar moment of inertia')
        self.Ixx = np.zeros(nFull) # self.add_output('Ixx', np.zeros(nFull), units='m**4', desc='area moment of inertia about x-axis')
        self.Iyy = np.zeros(nFull) # self.add_output('Iyy', np.zeros(nFull), units='m**4', desc='area moment of inertia about y-axis')
