...
def SegIntersect(A1,A2,B1,B2):
    ...
    OUTPUT:
        ...
        OR  -NaN """
    import numpy as np
    junk=np.cross(A2-A1,B2-B1,0,0,0)
    nA = np.sum(np.cross(B2-B1,A1-B1,0,0,0)*junk,axis=0);
    nB = np.sum(np.cross(A2-A1,B1-A1,0,0,0)*junk,axis=0);
    d  = np.sum(junk**2,axis=0);

    A0=np.zeros(A1.shape)*np.NaN
    B0=A0.copy()
    idx=np.nonzero(d)[0]
